...
const ICON = {
  ...
  cloud:   '<path d="M12 2v8"/><path d="m16 6-4 4-4-4"/><path d="M8 16H7a4 4 0 0 1 0-8 5 5 0 0 1 9.7-1.7A4.5 4.5 0 0 1 17 16h-1"/>',
...
/* ==========================================================================
   ...
   ========================================================================== */
const nav = $('#nav'), navInner = $('#navInner');
const REST = ['max-w-7xl', 'px-0'];
const PILL = ['max-w-6xl', 'px-6', 'py-3', 'bg-card/80', 'backdrop-blur-lg', 'shadow-lg', 'border-border', 'rounded-full'];
...
if (menu && toggle) {
  const iMenu = $('[data-icon="menu"]', toggle), iClose = $('[data-icon="close"]', toggle);
  ...
  toggle.addEventListener('click', (e) => { e.stopPropagation(); setMenu(!isOpen()); });
  menu.addEventListener('click', (e) => { if (e.target.closest('a')) setMenu(false); });
  document.addEventListener('click', (e) => {
    if (isOpen() && !e.composedPath().includes(menu) && !e.composedPath().includes(toggle)) setMenu(false);
  });
...
/* ==========================================================================
   ...
   ========================================================================== */
const SERVICES = [
  { k: 'code',    name: 'Product engineering', hue: 'brand',
  ...
  { k: 'chart',   name: 'Data & analytics', hue: 'teal',
  ...
  { k: 'cloud',   name: 'Cloud & DevOps', hue: 'light',
  ...
  { k: 'sparkle', name: 'AI integration', hue: 'indigo',
  ...
  { k: 'users',   name: 'Team augmentation', hue: 'accent',
...
const HUE = {
  brand:   { tile: 'bg-brand',      soft: 'bg-brand/10',      ring: 'ring-brand/15',      text: 'text-brand' },
  accent:  { tile: 'bg-accent',     soft: 'bg-accent/10',     ring: 'ring-accent/15',     text: 'text-accent-strong' },
  light:   { tile: 'bg-brand-light',soft: 'bg-brand-light/10',ring: 'ring-brand-light/15',text: 'text-brand-light' },
  indigo:  { tile: 'bg-indigo-600', soft: 'bg-indigo-600/10', ring: 'ring-indigo-600/15', text: 'text-indigo-600' },
  teal:    { tile: 'bg-teal-600',   soft: 'bg-teal-600/10',   ring: 'ring-teal-600/15',   text: 'text-teal-600' },
};

const selector = $('#selector'), panel = $('#panel');
let active = 0;

function renderSelector() {
  selector.innerHTML = SERVICES.map((s, i) => {
    const on = i === active, h = HUE[s.hue];
    return `<button role="tab" id="tab-${i}" aria-selected="${on}" aria-controls="panel" tabindex="${on ? 0 : -1}" data-i="${i}"
      class="group flex w-full items-center gap-4 rounded-2xl border p-4 text-left transition-all duration-300
             ${on ? 'border-border bg-card shadow-[0_15px_40px_-15px_rgba(31,93,160,.20)]'
...
function renderPanel() {
  const s = SERVICES[active], h = HUE[s.hue];
  panel.innerHTML = `
...
if (selector && panel) {
  const select = (i) => { active = i; renderSelector(); renderPanel(); };
  renderSelector(); renderPanel();
...
/* ==========================================================================
   ...
   ========================================================================== */
const STACK = ['TypeScript', 'React', 'Next.js', 'Node.js', 'Python', 'PostgreSQL', 'AWS', 'Terraform', 'Docker', 'Kubernetes', 'React Native', 'GraphQL'];
const marquee = $('#marquee');
if (marquee) {
  const item = (n) => `<div class="flex shrink-0 cursor-default items-center gap-3 font-display text-2xl font-bold text-border transition-colors hover:text-primary">${svg('layers', 'size-6')}${n}</div>`;
...
/* ==========================================================================
   ...
   ========================================================================== */
const BENTO = [
  { k: 'clock',  hue: 'brand',    span: 'lg:col-span-2', title: 'A written proposal in 48 hours',
...
if (bento) {
  ...
  if (!REDUCED) $$('[data-spot]', bento).forEach(el => {
...
/* ==========================================================================
   ...
   ========================================================================== */
const STEPS = [
  { k: 'search', n: '01', title: 'Consult',  when: 'Day 0',      body: 'A free 30-minute call. You describe the problem; we tell you whether we are the right team for it.' },
  { k: 'doc',    n: '02', title: 'Proposal', when: 'Within 48h', body: 'Scope, price, timeline and assumptions in writing. Fixed price or transparent hourly — your choice.' },
  { k: 'code',   n: '03', title: 'Build',    when: 'Weekly',     body: 'Two-week cycles with a working demo every week, in your repos and your accounts from commit one.' },
  { k: 'rocket', n: '04', title: 'Handover', when: '+30 days',   body: 'Documentation, a walkthrough with your team, and 30 days of support after go-live.' },
];
...
/* ==========================================================================
   ...
   ========================================================================== */
const CASES = [
  { sector: 'Logistics', hue: 'brand',    title: 'Case study one', body: 'What the problem was, what shipped, and the one number that moved.' },
  { sector: 'Fintech',   hue: 'teal',    title: 'Case study two', body: 'What the problem was, what shipped, and the one number that moved.' },
  { sector: 'Health',    hue: 'indigo',  title: 'Case study three', body: 'What the problem was, what shipped, and the one number that moved.' },
];
...
if (particles && !REDUCED) {
  ...
  particles.innerHTML = Array.from({ length: 24 }, () => {
    const size = 1 + rnd() * 3, dur = 8 + rnd() * 10;
...
/* ==========================================================================
   ...
   ========================================================================== */
const ph = $('#phToggle'), phLabel = $('#phLabel');
if (ph && phLabel) {
